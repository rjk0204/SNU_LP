...
from recognition.execute import do_recognition, decode_province
...
class DetectLP:
    def __init__(self):
        ...
        self.transform = None
        self.sort_tracker = Sort(max_age=300, min_hits=10, iou_threshold=0.1)
    ...
    def recognize(self, img_tensor, bbox):
        ...
        for b in bbox:
            ...
            if (x2 - x1) > 0 and (y2 - y1) > 0:  # Valid bounding box
                valid_bbox.append(b)

        if len(valid_bbox) == 0:
            return []

        recog_result = do_recognition(self.args, img_tensor, valid_bbox, self.recognition_network, self.converter, self.device)
        ...
        for bbox_idx in range(bbox_num):
            ...
            if len(cur_lp) >= 4:
                x1, y1, x2, y2 = int(cur_lp[0]), int(cur_lp[1]), int(cur_lp[2]), int(cur_lp[3])
                conf = cur_lp[4] if len(cur_lp) > 4 else None
                cls = cur_lp[5] if len(cur_lp) > 5 else None
